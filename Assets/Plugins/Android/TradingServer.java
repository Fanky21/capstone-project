...
public class TradingServer extends NanoHTTPD {
     ...
     */
    public TradingServer(Context context, String streamingAssetsPath, MoneyManager sharedMoneyManager)
            throws IOException {
        super(PORT);
        this.context = context;
    ...
    private Response handleMoneyOperation(String uri, Method method, IHTTPSession session)
        ...
        // Get POST data from files map (NanoHTTPD stores it here after parseBody)
        ...
        if (postData == null || postData.isEmpty()) {
            ...
            if (params.containsKey("amount")) {
                postData = "{\"amount\":" + params.get("amount") + "}";
            } else {
                return createJsonResponse(new JSONObject()
                        .put("success", false)
        ...
        android.util.Log.i("TradingServer", "Money operation POST data: " + postData);
        ...
        if (uri.equals("/api/unity/money/add")) {
            ...
            android.util.Log.i("TradingServer", "Added " + amount + ", new balance: " + moneyManager.getCash());
        } else if (uri.equals("/api/unity/money/subtract")) {
            moneyManager.subtractCash(amount);
            android.util.Log.i("TradingServer", "Subtracted " + amount + ", new balance: " + moneyManager.getCash());
        } else if (uri.equals("/api/unity/money/set")) {
            moneyManager.setCash(amount);
            android.util.Log.i("TradingServer", "Set to " + amount);
        } else {
            return createJsonResponse(new JSONObject()
                    .put("success", false)
