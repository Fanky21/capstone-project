...
function showStockDetail(company) {
    ...
    const owned = portfolio.portfolio.find(p => p.ticker === company.ticker);
...
function openTradingModal(mode) {
    ...
    if (mode === 'sell') {
        const owned = portfolio.portfolio.find(p => p.ticker === selectedStock.ticker);
...
async function handleTradeConfirm() {
    ...
    try {
        ...
        // Only show stock detail if we're in market view, otherwise stay in portfolio view
